...
class ClassesController extends Controller
{
    ...
    public function fetch(){
        $classes = Classe::join('ecoles', 'ecoles.id', '=', 'classes.ecole_id')
                    ...
                    ->select('regions.nom_reg', 'cantons.nom_cant', 'villages.nom_vill', 'ecoles.nom_ecl', 'classes.id', 'classes.nom_cla', 'classes.effec_fil', 'classes.effec_gar')
    ...
    public function store(Request $request)
    {
        ...
        for ($i=0; $i < count($nom_cla) ; $i++) { 
            ...
            if($ver === 0){
                ...
                $data['effec_gar'] = $request->effec_gar[$i];
                $data['effec_fil'] = $request->effec_fil[$i];
    ...
    public function show($id)
    {
        ...
        ->join('prefectures', 'prefectures.id', '=', 'communes.prefecture_id')
        ->join('regions', 'regions.id', '=', 'prefectures.region_id')
        ->select('regions.nom_reg', 'prefectures.nom_pref', 'communes.nom_comm', 'cantons.nom_cant', 'villages.nom_vill', 'ecoles.nom_ecl', 'classes.id', 'classes.nom_cla', 'classes.effec_fil', 'classes.effec_gar')
        ->findOrFail($id);
