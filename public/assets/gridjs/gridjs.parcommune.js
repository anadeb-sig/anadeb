...
const id = params.get('id');
  ...
  fetch("ecole_par_commune/"+id)
    .then(response => response.json())
    .then(data => {
      // Calculer la somme de la deuxième colonne
      const som_pre = data.reduce((acc, row) => acc + parseInt(row.nbr_pre), 0);
      const som_pri = data.reduce((acc, row) => acc + parseInt(row.nbr_pri), 0);
